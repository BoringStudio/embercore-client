mod device_input_state;

use winit::dpi::PhysicalPosition;
use winit::event::*;

use self::device_input_state::*;
    ...
    pub fn handle_window_event(&mut self, event: &WindowEvent) {
        match event {
            WindowEvent::KeyboardInput {
                ...
                match state {
                    ElementState::Pressed => self.keyboard.current.press(key),
                    ElementState::Released => self.keyboard.current.release(key),
                }
            }
            WindowEvent::MouseInput { button, state, .. } => match state {
                ElementState::Pressed => self.mouse.current.press(button),
                ElementState::Released => self.mouse.current.release(button),
            },
